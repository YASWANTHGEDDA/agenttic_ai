...
import { FiRefreshCw, FiEdit2, FiTrash2, FiPlayCircle, FiSave, FiX } from 'react-icons/fi';
import { getUserFiles, renameUserFile, deleteUserFile, analyzeDocument } from '../services/api'; // getUserFiles comes from here
...
const FileManagerWidget = ({ refreshTrigger, onAnalysisComplete }) => {
  ...
  const defaultAnalysisProvider = 'groq_llama3'; // Or your preferred default
  ...
  const fetchUserFiles = useCallback(async () => {
    ...
    setError('');
    try {
      // 'apiResponse' here IS the JSON data from the server,
      // because api.js's getUserFiles does .then(res => res.data)
      const apiResponse = await getUserFiles();
      console.log('FileManagerWidget: Fetched user files API response:', apiResponse);

      // Now, determine the actual structure of 'apiResponse'.
      // Common structures from a Node.js backend proxying Python might be:
      // 1. Direct array: `[ {file1}, {file2} ]`
      // 2. Object with a 'files' key: `{ files: [ {file1}, {file2} ] }`
      // 3. Object with a 'data' key (if Python wrapped its own response): `{ data: [ {file1}, {file2} ] }`

      if (Array.isArray(apiResponse)) {
        // Case 1: Server sends the array directly
        setUserFiles(apiResponse);
        console.log(`FileManagerWidget: Set ${apiResponse.length} files directly from array response.`);
      } else if (apiResponse && Array.isArray(apiResponse.files)) {
        // Case 2: Server sends { files: [...] }
        setUserFiles(apiResponse.files);
        console.log(`FileManagerWidget: Set ${apiResponse.files.length} files from response.files.`);
      } else if (apiResponse && Array.isArray(apiResponse.data)) {
        // Case 3: Server sends { data: [...] } (less common if Node directly proxies Python's list)
        setUserFiles(apiResponse.data);
        console.log(`FileManagerWidget: Set ${apiResponse.data.length} files from response.data.`);
      } else {
        console.warn('FileManagerWidget: getUserFiles response is not in an expected format (array, {files: []}, or {data: []}). Response:', apiResponse);
        setUserFiles([]); // Set to empty array if format is unrecognized
      }

    } catch (err) {
      console.error('FileManagerWidget: Error fetching files:', err);
      setError(err.message || 'Failed to load files.'); // err.message is formatted by api.js interceptor
      setUserFiles([]);
    } finally {
      setIsLoading(false);
    }
  }, []); // Empty dependency array for useCallback, as it has no external dependencies itself.

  useEffect(() => {
    console.log('FileManagerWidget: refreshTrigger changed or component mounted. Fetching files. Trigger value:', refreshTrigger);
    fetchUserFiles();
  }, [refreshTrigger, fetchUserFiles]); // fetchUserFiles is stable due to useCallback

  // --- handlers for rename, delete, analysis ---
  ...
  const handleAnalyzeClick = (file) => { 
    setAnalyzingFile({ serverFilename: file.serverFilename, originalName: file.originalName, filePath: file.filePath /* Pass filePath if available and needed by analyzeDocument */ }); 
  ...
  // userId is handled by Node.js middleware (tempAuth) for the /api/analysis/document route
  // file_path_for_analysis is constructed by Node.js in analysis.js

  try {
    const payloadForNode = {
      documentName: analyzingFile.originalName, // Node.js analysis.js expects documentName
      serverFilename: analyzingFile.serverFilename, // Node.js analysis.js expects serverFilename
      analysisType: selectedAnalysisType,       // Node.js analysis.js expects analysisType
      llmProvider: analysisLlmProvider,         // Node.js analysis.js expects llmProvider
      llmModelName: analysisLlmModel || null,       // Node.js analysis.js expects llmModelName
    };
    console.log("FileManagerWidget: Sending analysis payload to Node.js /api/analysis/document:", payloadForNode);
    
    // analyzeDocument function from api.js calls /api/analysis/document
    const responseData = await analyzeDocument(payloadForNode); 
    console.log("FileManagerWidget: Analysis API responseData (from Node.js):", responseData);

    // The responseData here is directly what analysis.js sends back after calling Python
    // which should be:
    // { documentName, analysisType, analysisResult, thinkingContent, status: 'success' }
    if (responseData && responseData.status === 'success') {
      if (onAnalysisComplete) {
        onAnalysisComplete({
          type: responseData.analysisType, // Use responseData.analysisType
          result: responseData.analysisResult,
          thinking: responseData.thinkingContent, // Use responseData.thinkingContent
          documentName: responseData.documentName // Use responseData.documentName
        });
      }
      setAnalyzingFile(null); // Clear the analysis state
    } else {
      throw new Error(responseData?.message || responseData?.error || "Analysis API call did not return expected success structure.");
    }
  } catch (err) {
    // err.message is already pre-formatted by api.js interceptor
    const errorMsg = err.message || "Failed to perform analysis.";
    console.error("FileManagerWidget: Analysis error:", err);
